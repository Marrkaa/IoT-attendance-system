  ...
  if (!node || !status) {
    return (
      <div className="card" style={{ padding: '1.5rem' }}>
        Pasirinkite IoT mazgą iš sąrašo.
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div className="card" style={{ padding: '1.25rem' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <span>Būsena: <strong>{status.status}</strong></span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Users size={18} color="var(--text-secondary)" />
            <span>Prijungta klientų: <strong>{status.connectedClients}</strong></span>
          </div>
          {status.lastPolled && (
            <span style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
              Atnaujinta: {new Date(status.lastPolled).toLocaleString('lt-LT')}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid var(--border)', fontWeight: 600 }}>
          Prijungti klientai (pagal paskutinį station dump)
        </div>
        {status.clients.length === 0 ? (
          <div style={{ padding: '1.5rem', color: 'var(--text-secondary)' }}>Nėra aktyvių klientų arba duomenys dar nebuvo gauti.</div>
        ) : (
                  ...
                  <tr key={c.macAddress}>
                    ...
                    <td>
                      {c.matchedStudentName ? (
                        ...
                        <span style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>neatpažinta</span>
                      )}
                    </td>
                  </tr>
                ))}
